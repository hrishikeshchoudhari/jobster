...
export const Resume: ResumeRelationResolvers = {
  applicant: (_obj, { root }) => {
    return db.resume.findUnique({ where: { id: root?.id } }).applicant()
  },
  educations: (_obj, { root }) => {
    return db.resume.findUnique({ where: { id: root?.id } }).educations()
  },
  employments: (_obj, { root }) => {
    return db.resume.findUnique({ where: { id: root?.id } }).employments()
  },
}
